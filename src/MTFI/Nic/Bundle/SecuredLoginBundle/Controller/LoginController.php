...
use MTFI\Nic\Bundle\SecuredLoginBundle\Interfaces\AuditableControllerInterface;
...
//        session destroys if user already logged in 
        ...
        return $this->render('NicSecuredLoginBundle:User:login.html.twig', array('form' => $form->createView(), 'error' => $error, 'csrf_token' => $csrfToken, 'browserSalt' => $salt));
    }

    public function preLoginAction($userName) {
        $em = $this->getDoctrine()->getManager();
        if (filter_var($userName, FILTER_VALIDATE_EMAIL)) {
            $preLoginUser = $em->getRepository('NicSecuredLoginBundle:User')->findOneByEmail($userName);
        } else {
            $preLoginUser = $em->getRepository('NicSecuredLoginBundle:User')->findOneByUsername($userName);
        }
        if ($preLoginUser) {
    ...
    public function expiredAction(Request $request) {
        return $this->render('NicSecuredLoginBundle:User:loginExpired.html.twig');
    }
    ...
    protected function renderLogin(array $data) {
        return $this->container->get('templating')->renderResponse('NicSecuredLoginBundle:Default:login.html.twig', $data);
    }
